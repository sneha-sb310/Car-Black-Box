...
unsigned char ext_eeprom_24c02_str_write(unsigned char addr, char *str);
# 17 "helper.c" 2
# 1 "./uart.h" 1
# 13 "./uart.h"
    ...
    if (i == 4) {
        ...
        if ((strncmp(user_pass, saved_pass, 4)) == 0) {
            ...
            if (attempts == 0)
            {
                ...
                wait_time = 60;
                while (wait_time != 0) {
                    clcd_putch((wait_time / 10 + '0'), (0xC0 + 10));
                    clcd_putch((wait_time % 10 + '0'), (0xC0 + 11));
                }
                clcd_print("secs", (0xC0 + 13));
                attempts = 3;
            } else {
                clear_screen();
                clcd_write(0x0C, 0);
...
unsigned char menu_screen(unsigned char reset_flag, unsigned char key) {
    ...
    if (menu_pos < 4) {
        ...
        clcd_putch('*', (0xC0 + 0));
        clcd_print(menu[menu_pos - 1], (0x80 + 1));
        clcd_print(menu[menu_pos], (0xC0 + 1));
    }
...
void display_log(unsigned char log_pos) {
    ...
    clcd_putch(saved_log[0], (0xC0 + 2));
    clcd_putch(saved_log[1], (0xC0 + 3));

    clcd_putch(':', (0xC0 + 4));

    clcd_putch(saved_log[2], (0xC0 + 5));
    clcd_putch(saved_log[3], (0xC0 + 6));

    clcd_putch(':', (0xC0 + 7));

    clcd_putch(saved_log[4], (0xC0 + 8));
    clcd_putch(saved_log[5], (0xC0 + 9));

    clcd_putch(saved_log[6], (0xC0 + 11));
    clcd_putch(saved_log[7], (0xC0 + 12));

    clcd_putch(saved_log[8], (0xC0 + 14));
    clcd_putch(saved_log[9], (0xC0 + 15));
...
void set_time(unsigned char reset_flag, unsigned char key) {
    ...
    if (!blink) {
        ...
        clcd_putch(newHr / 10 + '0', (0xC0 + 0));
        clcd_putch(newHr % 10 + '0', (0xC0 + 1));
        clcd_putch(':', (0xC0 + 2));

        clcd_putch(newMin / 10 + '0', (0xC0 + 3));
        clcd_putch(newMin % 10 + '0', (0xC0 + 4));
        clcd_putch(':', (0xC0 + 5));

        clcd_putch(newSec / 10 + '0', (0xC0 + 6));
        clcd_putch(newSec % 10 + '0', (0xC0 + 7));
    } else {

        clcd_putch(0x5F, (0xC0 + j));
        clcd_putch(0x5F, (0xC0 + j + 1));
    }
}
...
unsigned char change_password(unsigned char reset_flag, unsigned char key) {
    ...
    } else if (j == 4)
    {
        if (!strncmp(password_1, password_2, 4)) {
            ...
            i = 0;
            j = 0;
            clcd_print("PASSWRD MISMATCH", (0x80 + 0));
